...
public class PubSubIntegrationTest extends AbstractSmackIntegrationTest {
    ...
    /**
     * Asserts that one receives a published item that contains an 'offline' message event , after subscribing to a
     * node.
     * 
     * @throws NoResponseException   if there was no response from the remote
     *                               entity.
    ...
    @SmackIntegrationTest
    public void receiveOfflineMessageEventItemTest() throws NoResponseException, XMPPErrorException, NotConnectedException, InterruptedException, ExecutionException, PubSubException
    {
        final String nodename = "sinttest-receive-published-item-nodename-" + testRunId;
        LeafNode publisherNode = pubSubManagerOne.createNode(nodename);
        try {
            ...
            final EntityBareJid subscriber = conTwo.getUser().asEntityBareJid();
            subscriberNode.subscribe( subscriber );
            Message message = new Message(subscriber);
            message.setFrom(conOne.getUser().asEntityBareJid());
            MessageEvent event = new MessageEvent();
            event.setOffline(true);
            String stanzaID = StanzaIdUtil.newStanzaId();
            event.setStanzaId(stanzaID);
            message.addExtension(event);
            final CompletableFuture<Stanza> result = new CompletableFuture<>();
            conTwo.addAsyncStanzaListener( result::complete, stanza -> stanza.toXML("").toString().contains(stanzaID));
            publisherNode.publish( new PayloadItem<SimplePayload>(null,new SimplePayload(message.toXML().toString())));
